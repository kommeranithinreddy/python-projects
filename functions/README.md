# Python Functions Practice

This repository contains my Python functions practice work and mini projects completed while learning core Python programming.

## Overview

I completed 153 practice exercises covering:

* Functions
* Functional Programming
  * Lambda
  * Map
  * Filter
* Nested Functions
* Closures
* Decorators
* Generators
* Generator Expressions
* Recursion
* Mini Projects

The goal of this repository is to build strong problem-solving skills, improve Python fundamentals, and develop clean coding practices.

## Repository Structure

```text
functions/
├── basic/
├── strings/
├── lists/
├── functional-programming/
├── nested_functions/
├── closures/
├── decorators/
├── generators/
├── generator-expressions/
├── recursion/
├── mini-projects/
```

## Topics Covered

### Basic Functions

* Arithmetic operations
* Number utilities
* Leap year checking
* Temperature conversion
* Maximum and minimum value problems

### String Functions

* Palindrome checking
* String reversal
* Character counting
* String manipulation exercises

### List Functions

* List sorting
* Duplicate removal
* Maximum and second-largest element
* Common elements between lists

### Functional Programming

* Lambda functions
* Map
* Filter
* Sorting with custom keys

### Nested Functions

* Function composition
* Validation logic
* Menu systems
* Variable scope

### Closures

* Counters
* Multipliers
* Bank balance tracker
* Login attempt tracker
* Permission checker

### Decorators

* Function wrapping
* Authentication simulation
* Retry decorator
* Caching decorator
* Execution timing
* Function metadata preservation

### Generators

* Custom generators
* Fibonacci generator
* Prime number generator
* Infinite generators
* Generator memory usage

### Generator Expressions

* Lazy evaluation
* any() and all()
* Nested generator expressions
* Aggregation operations

### Recursion

* Factorial
* Fibonacci
* Binary Search
* Palindrome Check
* Flatten Nested List
* Sum and Product operations

### Mini Projects

* ATM Simulator
* Contact Book
* Email Validator
* Password Validator
* Student Grade System
* To-Do List Application
* Number Guessing Game
* Rock Paper Scissors
* Library Management System

## Learning Outcome

Through these exercises I developed:

* Strong understanding of Python functions
* Problem-solving skills
* Recursive thinking
* Functional programming concepts
* Code organization and modular design
* Practical application development skills

## Progress

This repository represents my completed Functions phase of Python learning before moving to:

- Exception Handling
- File Handling
- Object-Oriented Programming (OOP)
- Data Analysis
- Data Science
